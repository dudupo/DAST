...
\subparagraph{Proof.} Initialization, for $i=1$ the claim is trival, let's use that as the induction base case for proving Conservation. Assume the correctness of the claim for any $i^{\prime} < i$. And consdier the $i$th iteration. By the induction assumption we have that $x \notin \{a_1 .. a_{i-2} \} $, and by the fact we have reached the $i$th iteration we have,   
