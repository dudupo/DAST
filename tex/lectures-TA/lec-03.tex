...
\paragraph{Heap Sort.}   
Combining the above we obtain a new sorting algorithm. Given an array, we could first heapify it (build heap from it) and then extract the elements by their order. As we saw, the construction requires linear time, and then each extraction cost $\log n$ time. So, the overall cost is $O\left( n\log n  \right)$. Correction follows immedatly from Build and Extract correction.  
