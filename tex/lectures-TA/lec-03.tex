...
Let's compute the running time of the construction algorithm. The algorithm makes two recursive calls on input at half the original size and then pays $\Theta\left( \log n \right)$ time to drop the first node. Hence by using the Master Theorem, we have obtained that: 
\begin{equation*}
  \begin{split}
