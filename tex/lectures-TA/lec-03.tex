...
\begin{enumerate}
  ...
  \item 1,4,2,5,6,3	     (Y)
\end{enumerate}
How mach is cost (running time) to compute the the min of $H$? (without change the heap). ($O\left( 1 \right)$). Assume that option 4 is our Superpharm Line, let's try to imagne how should we maintain the line. After sereving the customer at top, what can we say about $ \{ H_{2}, H_{3}\}$? What can say about $\{H_{i} : i > 3 \}?$ (the second heighset value is in $\{H_{2}, H_{3} \}$.)   

\paragraph{Subtask: Extracting Heap's Minimum.} \textit{Let $H$ be an Heap at size $n$, Write algorithm which return $H_1$ and $H^\prime$ Heap which contain all the elemenets of $H / H_1 $. } 
