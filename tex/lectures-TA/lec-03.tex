...
\paragraph{Task: The Superpharm Problem.}\textit{you are requested to maintain a pharmacy line, in each turn you get one of the fallowing queries, etheir a new customer enter into the shop, ethir apharmacist request for the next person stand in front. In addtion, diffrent customers have difftent priorites, So you are asked to gurnte that in each turn the person with the heigset priority will be at the front. }

Before we condsider a sophisticated soultion, What is the running time for the naive soultion? (maintaining the line as linear array) ($\sim O\left( n^2 \right)$).  
...
\subsection*{Heaps.} Heaps are strucutres that in addition for supporting adding and removing elements are also enable to compute the maximum effincintly.  
...
\begin{defbox}{Heap}
  Let $n \in \mathbb{N}$ and consider the sequance $H = H_{1}, H_{2} \cdots H_{n} \in \mathbb{R} \left( * \right)$. we will say that $H$ is a Heap if for every $i \in [n]$ we have that: $H_{i} \le H_{2i}, H_{2i + 1}$ when we think of the value at indecis greater than $n$ as $H_{i>n} = -\infty$. 
\end{defbox}

\paragraph{Checking vital sigens.}Are the following sequances are heaps? 
\begin{enumerate}
  ...
  \item 1,4,2,5,6,3	     (Y)
\end{enumerate}
How mach is cost (running time) to compute the the min of $H$? (without change the heap). ($O\left( 1 \right)$). Assume that option 4 is our Superpharm Line, let's try to imagne how should we maintain the line. After sereving the customer at top, what can be said on $ \{ H_{2}, H_{3}\}$? or $\{H_{i>3}\}?$ (the second heighset value is in $\{H_{2}, H_{3} \}$.)   

\paragraph{Subtask: Extracting Heap's Minimum.} \textit{Let $H$ be an Heap at size $n$, Write algorithm which return $H_1$, erasie it and retuens $H^\prime$, an Heap which contain all the remain elemenets.} 
\textbf{Solution:} 
...
\end{algorithm}
\end{algbox}
\paragraph{Claim.} Assume that $H$ satisfiey the Heap inquality for all the elements except $H_{j}$. Namely for any $i \neq j$ we have that $H_{i} \le H_{2i}, H_{2i+1}$. Then applying Heapify-down on $H$ at index $j$ returns an heap. 
\paragraph{Proof.} Induction on $n - j$. Base case, $n-j < n/2 \Rightarrow j > n/2$, Hence $2j,2j+1 \notin [n]$ and we have that the Heap propety holds for any $i$.

Assume the correctness of the claim for any $j^\prime$ satisfy $j^\prime > j$ and consider $j$. 
...
\If{ \text{perent} $  > 0 \text{ and }  H_{\text{perent}} \le H_{i}$ } { 
