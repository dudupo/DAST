...
\begin{proof}
  If $e \in T$ then $F \cup \{ e \} \subset T$ and we done. So consider the second case $ e \notin{T} $. The intersection $T\cap X$ must not be empty, otherwise there is no path connecting vertices on the opposite ends of the cut (In notations of \Cref{def:cut}, no path connects vertices in $U$ to vertices in $\bar{U}$), and that is contradiction for $T$ be a spanning tree. Denote by \marginnote[ssddd ]{ndd }$e^{\prime} \in T \cap X$. And consider $T^{\prime} = T / \{e^{\prime}\} \cup \{e\}$.
