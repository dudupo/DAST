...
In the lecture you saw the Kruskal algorithm for finding an MST. The algorithm construct the MST in iteratively manner, In each step it holds a forest contained in MST and then looks for an edge which is minimal in some cut that it respects. Clearly, since $F$ has no cycles, if an edge $e \subset E$ doens't close a cycle at $F$ then there is a cut $X$ containing $e$ which respected by $F$.     
