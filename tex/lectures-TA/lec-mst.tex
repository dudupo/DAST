...
\end{claim}
\begin{proof}
  Assume trough contradiction that a graph $G^{\prime} = G / \{ e \} $, where $e \in C$ is not connected. Means that there are two vertices $u$ and $v$ that has a path between them in $G$ but, no such a path in $G^{\prime}$ denote that path by $P$ and observes that $e \in P$ otherwise $P$ would be also $u \rightsquigarrow v$ path in $G^{\prime}$. 
\end{proof}
