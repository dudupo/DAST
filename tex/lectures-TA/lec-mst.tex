...
If $e \in T$, then $F \cup \{ e \} \subset T$ and we are done. Otherwise, consider the second case where $e \notin T$. This means that $T \cup \{ e \}$ has $|V|$ edges and therefore must have a cycle. Let $\Gamma = T \cup \{ e \}$. Let $x$ and $y$ be the endpoints of $e$ (namely $e = (x,y)$). Denote the subset of vertices defining the cut $X$ by $U$. Since $T$ is connected, there is a path $x \rightsquigarrow y$ in $T$, denoted by $\mathcal{P}$. Additionally, because $e \notin T$, we have that $e \notin \mathcal{P}$. This means that there must be another edge in $\mathcal{P}$ connecting a vertex in $U$ to a vertex in $\bar{U}$\footnote{Otherwise, walking along $\mathcal{P}$ cannot take one out of $U$, leading to a contradiction as $\mathcal{P}$ leads to $y$.}.
  ...
  By using the fact that subtracting an edge from a cycle dosn't harm connectivity \Cref{claim:subtract}, we have that $\Gamma/\{e^{\prime}\}$ is connected, and since it has $|V|-1$ edges we conclude that it has to be a spanning tree. On the otherhand, by :   
  \begin{equation*}
    \begin{split}
