...
\caption{bucket-sort($A$, $n$)}
  ...
  Denote by $X_{i} : [n] \rightarrow [n]$ then random variable that counts the number of elements fallen in the $i$th bucket and by $X_{i}^{j}$ the indicator that $A_{i}\in B_{i}$. The Expectation of the sorting running time is:   
