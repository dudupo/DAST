  ...
  \begin{enumerate} 
    ...
    \item $P$, called the probability function, is a function that assigns a number in $[0,1]$ to any event, denoted as $P : 2^\Omega \rightarrow [0,1]$, and satisfies:
      \begin{enumerate}
        ...
        \item Normalization over the atomic events to $1$, which means $\sum_{\omega\in\Omega}P(\omega)i~=~1$.
      \end{enumerate}
  \end{enumerate}
\end{definition}
\begin{example}
...
\end{equation*}
\end{definition}
\begin{example} Consider rolling a pair of dices, denote by $X : [6]\times [6] \rightarrow [6] $ the random variable that set to be the result of the first roll, and let $Y$ be defined the all most the same but setting the result of the second dice. Namely if we denote by $\{(i,j)\}$ the atomic event associated with sample $i$ face on the first dice and $j$ face on the second then:  
  \begin{equation*}
    \begin{split}
      X(\{i,j\}) &= i \\ 
      Y(\{i,j\}) &= j  
    \end{split}
  \end{equation*}
  In addition, one can define the random variable $z$ to be their the sum, $Z = X+Y$, the sum is also a function  $\Omega \rightarrow \mathbb{R}$, and therefore $Z$ is also a random variable.  
\end{example}
