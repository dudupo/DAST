...
\begin{equation*}
  \begin{split}
    ...
    & \ \ \ \ + X_{k} \cdot 1  + \sum_{m > k}{ X_{m} \cdot T(m, k)} \\ 
     \Rightarrow \expp{T(n,k)} \le & cn +2c  \\
    & \ \ \ \  \ + \sum_{m < k}{ \expp{X_{m} \cdot T(n-m, k -m)}}  + \sum_{m > k}{\expp{ X_{m} \cdot T(m, k) }} \\ 
...
\end{claim}
\begin{proof}
  We will prove that for $k=i+1$, the sum is greater than $k=i$ if $i<\lfloor n/2 \rfloor$. Denote by $S_{i} =\sum_{m < i}{ \frac{n-m}{n} }  + 2c\sum_{m > i}{ \frac{m}{n}}$, Then the substitute $S_{i+1} - S_{i}$ become    
  \begin{equation*}
    \begin{split}
