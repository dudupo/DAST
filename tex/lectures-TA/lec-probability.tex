...
\begin{equation*}
  \begin{split}
    T(n,k) &= c\cdot n + \sum_{m < k}{ X_{m} \cdot T(n-m, k -m)} + X_{k} \cdot 1  + \sum_{m > k}{ X_{m} \cdot T(m, k)} \\ 
    & \le c\cdot n +  c \sum_{m < k}{ \frac{n-m}{n} }  + c\sum_{m > k}{ \frac{m}{n}} \\
    & =c \cdot n + c \cdot \sum_{m<k}{1} + c \cdot \sum_{m>k}{1}\\
    &= c\cdot n + c(n-1)+c = 2c n  
  \end{split}
\end{equation*}
