...
\begin{example}
  ...
  \marginnote[Note 1: Despite the ease of computing the expectation, calculating the exact probability of $\sum_{i}\sum_{j}X_{i}^{1}X_{i}^{j} = L$ for an arbitrary $L$ is a difficult task.]{Note 1: Despite the easyness of computing the expectation, Computing the exact probability of  $\sum_{i}\sum_{j}X_{i}^{1}X_{i}^{j} = L$ for some arbiray $L$ is an hard task.}
