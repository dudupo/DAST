...
\begin{equation*}
  ...
  %& \le c\cdot n + +2c + (n-1)\cdot 2c\cdot \frac{1}{n} \cdot \max \{ k , n -  k  \}\\
   % & \le 2c \cdot n 
  \end{split}
\end{equation*}
...
\end{claim}
\begin{proof}
  We will prove that for $k=i+1$, the sum is greater than $k=i$ if $i<\lfloor n/2 \rfloor$. Denote by $S_{i} =\sum_{m < i}{ \frac{n-m}{n} }  + \sum_{m > i}{ \frac{m}{n}}$, Then the substitute $S_{i+1} - S_{i}$ become    
  \begin{equation*}
    \begin{split}
      S_{i+1} - S_{i} = \frac{n-i - 1}{n} - \frac{i}{n} = \frac{n-2i-1}{n}
    \end{split}
  \end{equation*}
  And that quantity is positive for any $i < \lfloor n/2 \rfloor$.
\end{proof}
...
\begin{equation*}
  \begin{split}
    ...
    = &  c\cdot n + 2c +  2 \cdot  2c \frac{ \lfloor n/2  \rfloor \lfloor n/2 - 1 \rfloor }{2n} \le cn + 2c + n/2 \cdot 2c  \le 2c \cdot n
  \end{split}
\end{equation*}
