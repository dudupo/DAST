    ...
    \begin{enumerate}
      \item So first we been asked whether $X_{1}^{1}\cdot  X_{1}^{2} = 1$, Since this happens only if both $X_{1}^{1} = 1$, $X_{1}^{2} = 1$ then by independently we have that: 
        \begin{equation*}
          ...
          \end{split}
        \end{equation*} Now, if we would like to answer whether the first and the second fall into the same cell then we have to ask if there is an $i$ such that $X_{i}^{1}\cdot  X_{i}^{2} = 1$, also notice that for any different $i,i^{\prime}$ the $X_{i}^{j}$ and $X_{i^{\prime}}^{j}$ are indicators of disjointness events, that because $j$ cannot be fallen to both $i$, $i^{\prime}$ cells. Therefore $X_{i}^{1}\cdot  X_{i}^{2}$ and $X_{i^{\prime}}^{1}\cdot  X_{i^{\prime}}^{2}$ are also indicators of disjointness events. Thus:  
        
        \begin{equation*}
          \begin{split}
            P(\exists i : X_{i}^1 \cdot X_{i}^{2} = 1) &= P( \bigcup_{i} X_{i}^1 \cdot X_{i}^{2} = 1) = \sum_{i}{P( X_{i}^1 \cdot X_{i}^{2} = 1)} \\
              &= m\cdot{1}{m^{2}} = \frac{1}{m}
          \end{split}
        \end{equation*} We basically done. Yet we want to present the same calculation in a different notation that later on will be useful for computing expectations. Observes that the random variable that counts to ''how many'' cells both the first and the second fall is $\sum_{i}{X_{i}^{i}\cdot X_{i}^{2} }$.     
