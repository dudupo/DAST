...
Let us introduce an example for the first case, we get an array $A_{1}, A_{2} .. A_{n}$ and asked to find the $k$ smallest element in it. Here we are going to suggest random algorithm that expected to returns at linear time even we are not assume any assumption on the input and in particular how is it distributed.   
...
\end{algorithm}
Consider the first call to 'select' and let $X_{m}$ be the indicator for selecting the index of the $m$th smallest number on line (4). Notice that $X_m$ and the running time of the recursive calls are independent random variables. Therefore, the expected running time is:
\begin{equation*}
  \begin{split}
