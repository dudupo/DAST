...
\begin{example}\ 
\begin{enumerate}
    \item \( T(n) =  T(\frac{2n}{3}) + 1 arrow f(n) = 1 =\Theta ( n^{\log_{\frac{3}{2}} (1)})\) matches the second case. i.e  \( T(n) = \Theta ( n^{\log_{\frac{3}{2}} (1)}\log n )\).
    
    \item \( T(n) = \TT{3}{4} + n\log n arrow f(n) = \Omega( n^{\log_{4}(3) + \varepsilon}  ) \) and notice that \( af( \frac{n}{b}) = \frac{3n}{4}\log(\frac{n}{4})\) . Thus, it's matching the third case. \(\Rightarrow T(n) = \Theta(n\log n)\).
    
    \item \(T(n) = 3T( n^{\frac{1}{3}}) + \log\log n\). 

      Let $ m = \log n $, thus: $$\Rightarrow T( n) = T (2^m ) = 3T(2^{\frac{m}{3}} ) + \log m $$  And denote by $ S = S(m) = T(2^m) $ so we get the recursive form: $$\rightarrow S(m) = 3T(2^{\frac{m}{3}} ) + \log m = 3S(\frac{m}{3} ) + \log m $$. And we can apply the master theorem according to the first case: $$\log m = O(m^{\log_{3}(3)-\varepsilon} ) \rightarrow T(n) = T(2^m) = S(m) = \Theta(m) = \Theta( \log(n)) $$
\end{enumerate}
\end{example}
