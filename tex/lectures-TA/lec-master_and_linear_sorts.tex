...
\begin{enumerate}
    ...
    \item \(T(n) = 3T( n^{\frac{1}{3}}) + \log\log n\). Let \( m = \log n \Rightarrow T( n) = T (2^m ) = 3T(2^{\frac{m}{3}} ) + \log m\).  Denote by \(S = S(m) = T(2^m) arrow S(m) = 3T(2^{\frac{m}{3}} ) + \log m = 3S(\frac{m}{3} ) + \log m\). And by the fact that \(\log m = O(m^{\log_{3}(3)-\varepsilon} ) arrow T(n) = T(2^m) = S(m) = \Theta(m) = \Theta( \log(n)) \).  
\end{enumerate}
\end{example}
