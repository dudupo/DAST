...
The master theorem is the result of the recursive expansion. It handles recursive functions at the form of \(T\left(n\right) = a\cdot T\left( \frac{n}{b} \right) + f\left(n\right) \), for a positive function \(f : \mathbb{N} \rightarrow \mathbb{R}^{+} \).       

\begin{framed} \noindent \textbf{Master Theorem.}\\
  First, Consider the case that \(f = n^c\). Let \( a \ge 1, b > 1\) and \( c \ge 0 \). Then: 
\begin{enumerate}
    ...
    \item if \(\frac{a}{b^c} > 1 \) then \( T\left(n\right) = \Theta \left( n^{\log_{b} \left(a\right)} \right) \) \ \ \ \textbf{(\(f\) loses)}.
  \end{enumerate}
\end{framed}
...
\begin{example}\
  ...
  \end{enumerate}
\end{example}
\begin{framed}\noindent \textbf{Master Theorem, General Version.}\\
Now, let's generalize the simple version for an arbitrary positive \(f\) and let~\(a~\ge~1~,~b~>~1\). 

\newcommand{\logab}{\log_{b} \left(a\right)}

\begin{enumerate}
    \item if  \(f\left(n\right) = O \left( n^{\logab - \varepsilon }\right)\) for some \( \varepsilon > 0 \) then~\(T\left(n\right)=\theta\left( n^{\logab}\right)\) \ \ \ \textbf{(\(f\)~loses)}.
...
\begin{example} Assume that the input is permutation over $\{1 ,2 ,3, .., n \}$. For example: \begin{equation*}
    \begin{split}
  A = 5,1,2,3,6,4,7,8
    \end{split}
  \end{equation*}
...
\end{example}
\begin{example} Assume that the input is an array containing only $0$s and $1$s. For example: \begin{equation*}
    \begin{split}
  A = 0,1,1,1,0,0,1,0,1
    \end{split}
  \end{equation*}
...
\textbf{Solution.} First, count the zeros. Let $x$ be the amount. Then print $0$ for $x$ times and print $1$ for $n-x$ times. This can be done in $\Theta(n)$ time.
\end{example}
...
Notice that the Counting sort can beat the lower bound of $\Omega\left(n \log n\right)$ only because it is not a comparison sort. In fact, no comparisons between input elements occur anywhere in the code. Instead, counting sort uses the actual values of the elements to 'index' them into an array.

An important property of the counting sort is that it is \textbf{stable}.

\begin{framed}\textbf{Stable Sort.}
 We will say that a sorting algorithm is stable if elements with the same value appear in the output array in the same order as they do in the input array. \end{framed}
