...
\begin{framed}
    ...
    \end{defn}
    \begin{defn} A tree's height $h(T)$ (sometimes $h(r)$) is defined to be the
    length of the
    longest
    simple path from $r$ to a leaf.
    \end{defn}
    \begin{defn} A binary search tree is a binary tree $(T,r)$ such that for any
...
\begin{framed}
    \begin{defn} An AVL tree is a balanced BST, such that for any node $x$, it's left
    ...
    (1)$:
    \begin{defn} For each node $x\in T$, it's Balance Factor is defined 
    $$hd
