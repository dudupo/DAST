...
\end{figure}\section*{Appendix}

How would look like an algorithm which serves the 'sumover' query? Noitce that if a vertex $v$ has key such $v$.key $< x$ then all the vertices with values less than $x$ 
are vertices of the left substree of $x$. Therefore if $v$.key $x.key$ then $\rightarrow$ \texttt{Sumover($v$, $x$)} $=$ \texttt{Sumover($v.left$, $x$)}.   
