...
Let us use the following notation: we say that line ($x$) of the recursive call 'Explore($G,v$)' is executed at time $t$ if at time $t$ we execute line number $x$ of the algorithm Explore where the vertex argument is $v$. Notice that since Explore is called only once on each vertex, we have that the notation is well defined.\\
