  ...
  \begin{proof} 
	...
	\begin{enumerate}
	  \item Either pre($v$) $<$ post($u$). In that case, we will prove, by induction on $\text{pre}(v)-\text{pre}(u)$, that for any $u,v$ satisfies the relations, the third case holds. 
		...
		\begin{enumerate}
		  \item Either $\text{post}(w) > \text{pre}(v)$. Observes that $\text{pre}(v) - \text{pre}(w) < \text{pre}(v) - \text{pre}(u) = t$, and also  $\text{pre}(w) - \text{pre}(u) < \text{pre}(v) - \text{pre}(u) = t$. Therefore by induction assumption $[\pre(v),\post(v)] \subset [\pre(w),\post(w)]\subset [\pre(u),\post(u)]$, and in addition $w$ is a descendant of $u$ and $v$ is a descendant of $w$. Hence $v$ is a descendant of $u$.  
		\end{enumerate}
	    \end{enumerate}
