...
\begin{proof}
  ...
  \begin{enumerate}
    ...
    \item Step. Consider vertices $v,u$ such $u\rightarrow v$, in $G_{i}$ and that $\pre(u) < \pre(v)$. Since the running of the algorithm over $G_{i}$ preceding the explore call of $v_{i}$ is identical to the running over $G_{i-1}$ it is left to prove where $v = v_{i}$. Let $v^{\prime}$ be predecessor to $v$ on that path, namely the last vertex on the path before $v$. 
      \begin{itemize}
        \item If $v^{\prime}=v_{i-1}$, then consider the call 'Explore($G_{i},v_{i-1}$)', since $\pre(v_{i-1}) = i-1  < \pre(v_{i})$, then at the beginning of the recursive call $v_{i}$ has not yet visited and the algorithm sets $\pi(v_{i}) \leftarrow v_{i-1}$, now using the induction assumption, $v_{i-1}$ is a descendant of $u$ in $\pi_{i-1} \subset \pi_{i}$ we have also that $v_{i}$ is a descendant of $u$ in $\pi_{i}$. 
          Else, If $v^{\prime} \neq v_{i-1}$. 
      \end{itemize}
  \end{enumerate}
\end{proof}
