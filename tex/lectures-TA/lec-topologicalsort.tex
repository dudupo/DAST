...
Suppose, toward a contradiction, that $\post(u) < \post(v)$, and split into cases:
  \begin{itemize}
    ...
    \item Or, $\pre(v) > \pre(u)$ thus, from the parenthesis theorem, $\pre(u) < \post(u) < \pre(v) < \post(v)$, and $u$ is not an ancestor of $v$ in $\pi$. Hence, there is no path from $u$ to $v$ in $G$ which contradicts the given. 
  \end{itemize}
\end{proof}
