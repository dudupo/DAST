...
\begin{enumerate}
  ...
  \item Step. Consider the $i$th iteration. 
    ...
    \begin{itemize}
        ...
        So after the swapping in line (3), we get that $A^{(i)}_{n-i+1}$ is the element which is greater than $n-i$ elements in $A$. By using the second part of the induction assumption, we know that it is also less than $A^{\prime (i-1) }_{n-i+2},A^{\prime (i-1) }_{n-i+3},..A^{\prime (i-1) }_{n}$, so after line (3) and by that $A^{\prime (i-1) }_{n-i+2},A^{\prime (i-1) }_{n-i+3},..A^{\prime (i-1) }_{n}$ are the $i-1$ largest elements placed in order, we have that:
