...
\section{Review.}
  \begin{definition}    
    Consider the sequence $H = H_{1}, H_{2}, \cdots, H_{n} \in \mathbb{R} \left( * \right)$. We will say that $H$ is a min-Heap if for every $i \in [n]$, we have that $H_{i} \le H_{2i}, H_{2i + 1}$\footnote{When we think of the values at indices greater than $n$ as $H_{i>n} = -\infty$}.
