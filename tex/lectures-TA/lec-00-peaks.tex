...
\begin{enumerate}
  ...
  Hence it must hold that $j^{\prime} < j$. But satisfaction of the condition on line (2) can happen only if $A_{j^{\prime}}$ is a peak, which contradicts the minimality of $j$. \marginpar[Note: text for left-hand side text]{Note 1: text for right-hand side of pages, it is set justified.}
