...
\begin{tikzpicture}
    ...
    \draw[domain=0:4,smooth,variable=\x,blue,name path=c1] plot ({\x},{ 1 })node[right]{$f_1(n)$};
    \draw[domain=4.5:6.5,smooth,variable=\x,red,name path=c2] plot ({\x},{ \x-4.5})node[right]{$f_2(n)$};
    \draw[domain=6.5:8.5,smooth,variable=\x,red,name path=c2] plot ({\x},{ 2-(\x-6.5)})node[right]{$f_2(n)$};
    \draw[domain=9:13,smooth,variable=\x,red,name path=c2] plot ({\x},{\x-9})node[right]{$f_2(n)$};
...
\begin{proof}
Assume, trough contradiction, that there exists an $n$-length array $A$ such that the algorithm peak-find fails to find one of its peaks. In particular, the algorithm either returns $j^{\prime} \in [n]$ such that $A_{j^{\prime}}$ is not a peak or does not return at all (never reaches line (3)).
\begin{enumerate}
