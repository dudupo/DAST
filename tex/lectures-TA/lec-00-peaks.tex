...
\begin{proof}
  ...
  Denote by $i$ the current iteration setp\footnote{ In our course, in terms of terminology, usually 'step' = 'iteration' = 'iteration number' = 'turn' = 'index'. } 
...
\begin{enumerate}
  \item Let's handle first the case in which returning indeed occurred., and note that if the algorithm gets to line (2) in the $j$th iteration then either it returns $j$ or $A_{j}$ is not a peak. 
