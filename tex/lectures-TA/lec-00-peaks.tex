...
\ctt{  
  In the previous version of the recitation, the first recursive call was to find-peak$\left(A_{i},A_{i+1}..A_{n}\right)$. However, in the case where $n=2$, we have that $A_{i}=1$, so it does not hold that $|A^{\prime}| < |A|$. And we cann't use the induction assumption. 
}
