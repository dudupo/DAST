...
{
   \fancyhf{}
   \fancyhead[RO, LE]{\textbf{The performance of new graduates}, \includegraphics[width = 4cm]{logo.jpg}}
   %\lhead{\includegraphics[width = 2cm]{logo.jpg}}
}
\begin{document}
\ifdefined\BOOK
...
\begin{proof}
  Denote by $j$ the position of the first peak in $A$, we will prove that \Cref{alg:simp} returns $A_{j}$. 
...
\If { $A_{1} \ge A_{2}$ or $n=1$ }{
 ...
 \begin{claim} \label{claim:subarray} 
   Let $A = A_1, \dots, A_n$ be an array, and $$A' = A^{\prime}_1, A^{\prime}_2, \dots, A^{\prime}_{n-1} =  A_2, A_3, \dots, A_n$$ be the $n-1$ length array obtained by taking all of $A$'s items except the first. If $A_1 < A_2$,\footnote{ weak inequality ($\le$) is sufficient.} then any peak of $A'$ is also a peak of $A$.
\end{claim}
\begin{proof} 
...
\begin{enumerate}
  ...
  \item If $j^{\prime} = 1$, then $A^{\prime}_{1} \ge A^{\prime}_{2} \Rightarrow A_{2} \ge A_{3}$ and by combining the assumption that $A_{1} < A_{2}$ we have that $A_{2} \ge A_{1}, A_{3}$. So $A_{2} = A^{\prime}_{1}$ is also a peak.
...
\begin{enumerate}
  ...
  \item Assumption. Assume that for any $m$-length array, such that $m<n$ the algorithm returns a peak. 
  \item Step. Consider an array $A$ of length $n$. If $A_1$ is a peak, then the algorithm answers affirmatively on the first check, returning $1$ and we are done. If not, namely $A_1 < A_2$, then by using \Cref{claim:subarray} we have that any peak of $A' = A_2, A_3, \dots, A_n$ is also a peak of $A$. 

    The length of $A'$ is $n-1 < n$. Thus, by the induction assumption, the algorithm succeeds in returning on $A'$ a peak which is also a peak of $A$.
\end{enumerate}

\section{An attempt for improved solution.}
...
\begin{algorithm}
\KwResult{returns a peak of \(A_1 ... A_n \in \mathbb{R}^n \)  }
\caption{fail attempt for more improved alg. }
...
The reason we didn't succeed, and actually do not have a chance, to complete the induction step is that we didn't enforce any relation between the subarray $A^{\prime}$ and its ancestor $A$. We would like to have a claim similar to \Cref{claim:subarray}. That brings us to define \Cref{alg:workreq} and use \Cref{claim:subarraymiddle} to prove its correctness.
\begin{algorithm}
\KwResult{returns a peak of \(A_1 ... A_n \in \mathbb{R}^n \)  }
\caption{improved algorithm}
$ i \leftarrow  \lceil n/2 \rceil $\\
        \If { $A_{i}$ is a peak in $A$ }{
          \Return $i$
        }
        \ElseIf { $i=1$ or $A_{i-1} \le A_{i}$ } { 
...
\begin{enumerate}
  \item Base. Single element array. Trivial. 
  \item Assumption. Assume that for any $m$-length array, such that $m<n$ the algorithm returns a peak. 
  \item Step. Consider an array $A$ of length $n$. If $A_{\lceil n/2 \rceil}$ is a peak, then the algorithm answers affirmatively on the first check, returning $\lceil n/2 \rceil$ and we are done.

  If not, then either $A_{\lceil n/2 \rceil} < A_{\lceil n/2 \rceil - 1}$ or $A_{\lceil n/2 \rceil} < A_{\lceil n/2 \rceil + 1}$. We have already handled the first case, that is, using \Cref{claim:subarraymiddle} we have that any peak of $A' = A_{\lceil n/2 \rceil + 1}, A_{\lceil n/2 \rceil + 2}, \dots, A_n$ is also a peak of $A$. The length of $A'$ is $ n - \lceil n/2 \rceil - 1 \le  \lfloor n/2 \rfloor < n$. So by the induction assumption, in the case where $A_{\lceil n/2 \rceil} < A_{\lceil n/2 \rceil - 1}$ the algorithm returns a peak. 


  In the other case, we have $A_{\lceil n/2 \rceil} < A_{\lceil n/2 \rceil + 1}$ (otherwise $A_{\lceil n/2 \rceil}$ would be a peak). 

  Finishing the proof is left as an exercise.
\end{enumerate}
...
\begin{enumerate}
  ...
  \item Step. Notice that in the worst case, $\lceil n/2 \rceil$ is not a peak, and the algorithm calls itself recursively immediately after paying $c_{2}$ in the first check.

    The recursive call is over an array length of $\lceil n/2 \rceil - 1$ \footnote{ Notice $ \lceil n/2 \rceil - 1 \le \lfloor n/2 \rfloor$.}. Hence: \begin{equation*}
      \begin{split}    
	T\left(n\right) & \le c_{2} + T\left(n/2\right) \le c_{2} + c_{1} \log\left( \lfloor n/2 \rfloor \right) + c_{2}\\
	& = c_{1} - c_{1} + c_{2} + c_{1} \log\left( \lfloor n/2 \rfloor \right) + c_{2}\\
	& = c_{1} \log (2) +  c \log\left( \lfloor n/2 \rfloor  \right) + 2c_{2} - c_{1} \\
	& = c_{1} \log\left(2 \left(\lfloor n/2 \rfloor \right) \right) + 2c_{2} - c_{1}
      \end{split}
    \end{equation*}
