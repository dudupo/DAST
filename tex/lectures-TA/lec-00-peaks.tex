...
\begin{enumerate}
  \item Let's handle first the case in which returning indeed occurred. Denote by $j$ the first position of a peak in $A$, and note that if the algorithm gets to line (2) in the $j$th iteration then either it returns $j$ or $A_{j}$ is not a peak.  \marginpar[Note: text for left-hand side text]{Note 1: text for right-hand side of pages, it is set justified.}
