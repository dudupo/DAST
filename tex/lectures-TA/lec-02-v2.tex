...
\section{Bounding recursive functions by hands.}
Our main tool for dealing with recursive relations is the Master Theorem, which was proven in the lecture. In order to gain a better understanding, let's revisit the calculation in the proof for a specific case. Suppose that your algorithm analysis has yielded the following recursive relation:
...
Our primary tool for handling recursive relations is the Master Theorem, which was proven in the lecture. To gain a better understanding, let's revisit the calculation in the proof for a specific case. Suppose that your algorithm analysis has resulted in the following recursive relation:
\begin{example}\( T\left(n\right)  = \left\{ \begin{array}{rcl}
& 4T\left(\frac{n}{2}\right)+c\cdot n & \mbox{for }  n > 1  \\
& 1 & \mbox{else}  
\end{array}\right. \). Therefore, the running time can be expressed as \begin{equation*}
    \begin{split}
 & T\left(n\right)  = 4T\left(\frac{n}{2}\right)+c\cdot n=  4\cdot4T\left(\frac{n}{4}\right)+4c\cdot\frac{n}{2}+c\cdot n = ... = \\ & \overset{\text{\textcolor{red}{critical}}}{\overbrace{4^{h}T(1)}} + c\cdot n\left(1+\frac{4}{2}+\left(\frac{4}{2}\right)^{2}...+\left(\frac{4}{2}\right)^{h-1}\right) = 4^{h} + c\cdot n\cdot\frac{2^{h}-1}{2-1}
    \end{split}
\end{equation*}
We will refer to the number of iterations until the stopping condition is reached as the recursion height, denoted by \(h\). What should the recursion height be? \( 2^{h} = n \Rightarrow h =\log\left(n\right) \). Therefore, the total algorithm running time is \( \Theta\left(n^2\right)\). 
\end{example}
\textbf{Question}: Why is the term \( 4^{h} T(1) \) so critical? Consider the case \(T\left(n\right) =  4T\left(\frac{n}{2}\right) + c \) .One common mistake is to forget the final term, resulting in a linear solution \( \Theta(n)\) instead of a quadratic one \( \Theta(n^2)\).

The Master Theorem is our primary tool for handling recursive relations, as proven in the lecture. To gain a better understanding, let's revisit the calculation in the proof for a specific case. Suppose your algorithm analysis has resulted in the following recursive relation:
\begin{example}\( T\left(n\right)  = \left\{ \begin{array}{rcl}
& 4T\left(\frac{n}{2}\right)+c\cdot n & \mbox{for }  n > 1  \\
& 1 & \mbox{else}  
\end{array}\right. \). Therefore, the running time can be expressed as \begin{equation*}
 ...
 & T\left(n\right)  = 4T\left(\frac{n}{2}\right)+c\cdot n=  4\cdot4T\left(\frac{n}{4}\right)+4c\cdot\frac{n}{2}+c\cdot n = ... = \\ & \overset{\text{\textcolor{red}{critical}}}{\overbrace{4^{h}T(1)}} + c\cdot n\left(1+\frac{4}{2}+\left(\frac{4}{2}\right)^{2}...+\left(\frac{4}{2}\right)^{h-1}\right) = 4^{h} + c\cdot n\cdot\frac{2^{h}-1}{2-1}
    \end{split}
\end{equation*}
We will refer to the number of iterations until the stopping condition is reached as the recursion height, denoted by \(h\). What should the recursion height be? \( 2^{h} = n \Rightarrow h =\log\left(n\right) \). Therefore, the overall algorithm running time is \( \Theta\left(n^2\right)\). 
\end{example}
\textbf{Question}: Why is the term \( 4^{h} T(1) \) so critical? Consider the case \(T\left(n\right) =  4T\left(\frac{n}{2}\right) + c \) . One common mistake is to forget the final term, resulting in a linear solution \( \Theta(n)\) instead of a quadratic \( \Theta(n^2)\).
    \begin{example}\( T\left(n\right)  = \left\{ \begin{array}{rcl}
