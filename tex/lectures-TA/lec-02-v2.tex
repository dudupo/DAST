...
\end{equation*}
\begin{example}
The function $T(n)$ is defined as follows:
\begin{equation*}
    T(n) = \begin{cases}
        4T\left(\frac{n}{2}\right) + c \cdot n, & \text{if } n > 1 \\
        1, & \text{otherwise}
    \end{cases}
\end{equation*}
The running time of this function can be calculated as:
\begin{equation*}
    \begin{split}
        T(n) &= 4T\left(\frac{n}{2}\right) + c \cdot n \\
        &= 4 \cdot 4T\left(\frac{n}{4}\right) + 4c \cdot \frac{n}{2} + c \cdot n \\
        &= ... \\
        &= 4^h T(1) + c \cdot n \left(1 + \frac{4}{2} + \left(\frac{4}{2}\right)^2 + ... + \left(\frac{4}{2}\right)^{h-1}\right) \\
        &= 4^h + c \cdot n \cdot \frac{2^h - 1}{2 - 1} \\
        &= 4^h + c \cdot n \cdot (2^h - 1)
    \end{split}
\end{equation*}
where $h$ is the number of recursive calls made before the base case is reached. 
\end{example}
