...
\begin{framed}{AVL Tree.} An AVL tree is a balanced BST, such that for any node $x$, its left and right subtrees' height differs in no more than $1$. In other words:
        $$|h(left(x)) - h(right(x))|\leq 1$$
    \end{framed}
    This field allows us to calculate the Balance Factor for each node in $O
    (1)$:

    \begin{framed}{Balance Factor.} For each node $x\in T$, it's Balance Factor is defined 
    $$hd
