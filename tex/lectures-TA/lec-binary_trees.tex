...
Arrays, Linked Lists, Heaps, and Hash Tables are all ways to maintain collections. Each offers a different trade-off in query running times. For some of them, we imposed good worst-case bounds, while for others we can only compute expected costs (Hash Tables). The obvious question is whether one could guarantee both fast access and update running times in the worst-case. 

In the following two lectures, we will that using Balanced Binary Search Trees, we can guarantee both Insertion/Deletion and access.
