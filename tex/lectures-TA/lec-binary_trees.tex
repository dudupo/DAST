...
\begin{framed}
  \begin{definition}Let $T$ be a binary search tree, and let $x$ be a node belonging to it. The \textbf{predecessor} of $x$ will be defined as a vertex $y$ such that $y$.key $\leq x$.key and $y$.key is maximal among the nodes satisfying this condition. If we were to set the values of $T$ in sorted order, then the predecessor of $x$ would be located on its left. The \textbf{successor} of $x$ will be defined as $y$, where $x$ is the predecessor of $y$.
  \end{definition}
\end{framed}
