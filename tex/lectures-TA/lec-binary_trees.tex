    ...
    \begin{enumerate}
      ...
      \item Step. Consider a binary tree over $n$ vertices with minimal hight. The longest path consists of an edge between the root and one of its children. Denote by $x$ the size of the root's left subtree, so the size of root's right subtree is $n-x$, Thus we get:
        \begin{equation*}
          ...
          \end{split}
        \end{equation*}
By the monotonic property of $H_{n}$, we have that the above is minimized when both $H_{x}=H_{n-x}=H_{n/2}$ if $n$ is even, or $\max(x,n-x) = \left\ceil n/2 \right\ceil$ when $n$ is odd. So we get:
        \begin{equation*}
          \begin{split}
            \ge & 1 + H_{\left\ceil n/2 \right\ceil} \\ 
            = & 1 + \log(\left\ceil \frac{n}{2} \right\ceil ) = \log(n+1)
          \end{split}
        \end{equation*}
    \end{enumerate}
