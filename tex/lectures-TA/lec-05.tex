...
\begin{itemize}
  \item Base. Where $d = 1$ the correctness followes imedatly from correctness of our base sort subroutine. 
  \item Induction Assumption. Assume that Radix-sort is correct for any array of numbers that each of them contain at most $d-1$ digits. 
  \item Step. Let $A^{\prime}$  be the algorithm output. Consider $x,y \in A$, Assume without loosing geanrality that $x > y$. Denote by $x_{d}, y_{d}$ their $d$-digit and by $x_{/d}, y_{/d}$ the numbers obtained by taking only the first  $d-1$ digits of $x,y$. Separate in two cases:

    \begin{itemize}
      \item   If $x_{d} > y_{d}$ then a secnario in which $x$ apper prior to $y$ is aimplay contrudiction to the correctness of our subroutine.
      \item   So consider the case in which $x_{d} = y_{d}$. In that case it must holdes that $x_{/d} > y_{/d}$. Then the appperance of $x$ prior to $y$ either contrudict the assumesption that the base algorithm we have used is stable, or that $x$ apper before $y$ in the end of the $d-1$ iteartion. Which contrudict the induction assumption. 
    \end{itemize}
 \end{itemize}
