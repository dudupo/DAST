...
\begin{enumerate}
  ...
  \begin{proof}
    ...
    \begin{enumerate}
      ...
      \item Step. Consider a vertex $u$ such that $\delta(u) = m$, and let $w$ be a vertex such that $\delta(w) > \delta(u)$. If $w$ does not enter the queue at all, then the statement holds trivially. Suppose it does, so $\pi(w)$ is defined. Let $y$ be $\pi(w)$, and we can assume that $\delta(y) = m$, otherwise we repeat the proof by taking $y$ instead of $w$ as the vertex at distance $\delta(y) > \delta(u)$. This is because, by definition, $\pi(w)$ is extracted before $w$, Thus proving that $\pi(w)$ extracted after $u$ is sufficient. 
