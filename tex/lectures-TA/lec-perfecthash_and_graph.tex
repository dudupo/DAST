...
\begin{enumerate}
  ...
  \begin{proof}
    ...
    \begin{enumerate}
      \item  Base. Consider $u$ and $w$ such that $\delta(u) = 0$ and $\delta(w) > 0$. In this case, $u$ is $v$, and the statement holds by definition. Furthermore, for any vertex $u$ that is a neighbor of $v$, the algorithm sets $\pi(u) \leftarrow v$, and it is easy to see that $\delta(u) = 1 = 1 + \delta(v)$.
