      ...
      \begin{enumerate}
        ...
        \item $j \ge k$ and $j<i+1$, We claim that for each such $j$ an exhange will always occuer. 
          ...
          \begin{proof}
            ...
            \begin{enumerate}
              \item Base. $A^{(1)}_{k}$ is greater than $A_{i}$, and be the previews case we have that at the begging of the $k$ iteration $A^{(k)}_{k}=A^{(1)}_{k}, A^{(k)}_{i}=A^{(1)}_{k}$. Therefore the condition on line (3) is satisfied, exchange is been made, and $A^{(k+1)}_{k} =A^{(k)}_{i} = A^{(0)}_{i}$ and $A^{(k+1)}_{i} = A^{(k)}_{k}$. Now  So $A^{(k+1)}_{k+1} = A^{(k)}_{k+1} = A^{(0)}_{k+1}$
