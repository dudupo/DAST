...
\input{../tex/lectures-TA/newcommands.tex}
\addbibresource{../tex/lectures-TA/sample.bib} 
...
\ctt{ 
  Noitce, sections numbers 2,3,4 are optional, you don't have to submit them but please make sure you feel capable to solve them. 
 }
...
\item Prove the correctness of Merge-Sort using your claim.
\end{enumerate}
\ifdefined\SOLUTION
...
\begin{equation*}
  ...
  \end{array} \right. 
\end{equation*}
When $\alpha, \beta, \gamma$ are positive numbers. Bound $T$ asymptotically tight, namely find $g: \mathbb{N} \rightarrow \mathbb{R}^{+}$ such that $T = \Theta(g)$. Notice that the solution depends on the relation between $\alpha, \beta, \gamma$.
% $\alpha/\beta^{c} > 1$
...
\section{$T_{1},T_{2}$-Running time.}
Let $T_{1},T_{2} : \mathbb{N} \rightarrow \mathbb{R}^{+}$ be functions satisfying the equations:
\begin{equation*}
  \begin{split}
    T_{1}\left( n \right) = 3T_{1}\left(\frac{n}{2} \right) + 2T_{2}\left(\frac{n}{2} \right) + n^{3}\\ 
    T_{2}\left( n \right) = 2T_{1}\left(\frac{n}{2} \right) + 3T_{2}\left(\frac{n}{2} \right) + n^{2} 
  \end{split}
\end{equation*}
