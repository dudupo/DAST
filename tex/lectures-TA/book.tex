...
\fi
 ...
 \section{Sorts That Assume Input Properties.}
...
\begin{definition}
  ...
  \begin{example} %\paragraph{Question.} 
    ...
    \end{enumerate}
    Let us define the indicator $X_{i}^{j}$ which indicate that the $i$th key fallen into the $j$th cell. 
    \begin{enumerate}
      \item So first we been asked whether $X_{1}^{1}\cdot  X_{2}^{1} = 1$
    \end{enumerate}
  \end{example}
