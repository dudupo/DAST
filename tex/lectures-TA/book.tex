...
%\begin{algorithm}{non-equal-merge alg.}
\begin{algorithm}
\SetAlgoLined
...
\begin{algorithm}{Maximum finding.}
\begin{algorithm*}[H]
% \SetAlgoLined
...
\begin{algorithm}{Element finding.}
\begin{algorithm}[H]
\SetKwInOut{Input}{input}
 \Input{ Array  $ a_1, a_2, .. a_n $  }
% \SetAlgoLined
...
\begin{algorithm}{Median-Insert-key.}
\begin{algorithm}[H]  
\SetKwInOut{Input}{input} 
...
\begin{algorithm}{Median-Extract.}
\begin{algorithm}[H]
\SetKwInOut{Input}{input}
 \Input{ Array  $ H_1, H_2, .. H_n $  }
% \SetAlgoLined
...
\begin{algorithm}{randomized-partition$(A, p, r)$}
  \begin{algorithm}[H]
    ...
    \end{algorithm}
\end{algorithm}
 \begin{algorithm}{randomized-quicksort $(A, p, r)$}
      \begin{algorithm}[H]
	\If{ $p < r$ }{
...
\begin{algorithm}{Partition$(A, p, r)$}
  \begin{algorithm}[H]
    $ x \leftarrow A_{r} $ \\
    $ i \leftarrow p - 1 $ \\
    \For{ $j \in [p, r-1]$ }{
...
\begin{algorithm}{Counting-sort$(A, n, k)$}
  \begin{algorithm}[H]
...
\begin{algorithm}{radix-sort(A, n, d)}
  \begin{algorithm}[H]
    \For{ $ i \in [1,d]$ } {
...
\begin{algorithm}{bucket-sort(A, n)}
  \begin{algorithm}[H]
...
\begin{algorithm}{AVL Insert(r,x)}
  \begin{algorithm}[H]
...
\begin{algorithm}{AVL-Leftsum Insert(r,x)}
  \begin{algorithm}[H]
    Call Insert $(r,x)$ // (The standard insertion routine for BST) \\  
    Let $p$ be the new node appended to the tree. \\
    \While{ $p$.parent $\neq \emptyset $ } {
