...
For this we present the following algorithm, which receives the connected graph $G$ and the starting node $s$ as input. Note that in addition to our definition of the graph $G$, we will now also save some property $dist$ for every vertex, and also a flag called $visited$.
...
\begin{enumerate}
    \item $v$.color is the color of $v$: WHITE, GRAY, or BLACK.
    \item $v$.d holds the distance from the source vertex $s$ to $v$,as computed by the algorithm.
    \item $v$.$\pi$ is $v$’s predecessor in the breadth-first tree. If $v$ has no predecessor because it is the source vertex or is undiscovered, then $v$.$\pi$ NIL.
\end{enumerate}
