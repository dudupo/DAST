...
\paragraph{Solution.}%We will associate with any vertex of the tree a field that counts the number of nodes whose keys are lower than it (the size of its left subtree). Let's denote it as left-size. The code is given in \Cref{alg:kstat}. Guidelline for proving correction, if root is strictly greater then $k-1$ elements, then the $k$-smallest element is in his left-subtree and in addition, it is also the the $k-1$ smallest element in that subtree. If the root is strictly lower then $k-1$ elements then it clears that the $k$-smallest element is in his right subtree, yet, in contrast to the previews case, here the order of $k$-smallest element of the whole tree in the subtree will be the subtitute between $k$ and the number of nodes in the left subtree $\cup$ the root. 
i%We will associate with each vertex of the tree a field that counts the number of nodes whose keys are lower than it (the size of its left subtree). Let's denote it as left-size. The code is given in \Cref{alg:kstat}. Guideline for proving correctness: if the root is strictly greater than $k-1$ elements, then the $k$-smallest element is in its left subtree and is also the $k-1$ smallest element in that subtree. If the root isgreater than strictly fewer than $k-1$ elements, then it is clear that the $k$-smallest element is also grather than it and located in its right subtree. However, in contrast to the previous case, here the order of the $k$-smallest element of the whole tree in the subtree will be the substitution between $k$ and the number of nodes in the $\{$ left-subtree $\cup$ root $\}$. The thired case, is the case that the root is grather than exactly $k-1$ eleemnts which by definition set him as the $k$ smallest element. 
We will associate with each vertex of the tree a field that counts the number of nodes whose keys are lower than it (the size of its left subtree). Let's denote it as left-size. The code is given in \Cref{alg:kstat}. Guideline for proving correctness: if the root is strictly greater than $k-1$ elements, then the $k$-smallest element is in its left subtree and is also the $k-1$ smallest element in that subtree. If the root is greater than strictly fewer than $k-1$ elements, then it is clear that the $k$-smallest element is also greater than it and located in its right subtree. However, in contrast to the previous case, here the order of the $k$-smallest element of the whole tree in the subtree will be the substitution between $k$ and the number of nodes in the $\{$ left-subtree $\cup$ root $\}$. The third case is when the root is greater than exactly $k-1$ elements, which by definition sets it as the $k$-smallest element.
\begin{algorithm}
