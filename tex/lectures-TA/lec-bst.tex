...
We have seen in the lecture that the runs of any sorting algorithm that doesn't assume anything on the input's structure, but is able to compare elements pair of it, can be modeled as a binary tree. On every node, the algorithm compares between two elements and based on the result, we advance our imaginary position to either the left or right child. Eventually, we get to the leaf of the tree and output. 

The height of the binary tree is (at least) the running time of the algorithm. We are about to demonstrate how one can have a lower bound over the find-peak problem.
