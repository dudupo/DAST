...
\paragraph{Solution.} 
  \begin{enumerate}
    \item Recall that any reverse-ordered sorted array is also a max heap. This is because if $A_{i} \ge A_{j}$ for any $i < j$, then it follows that $A_{i} \ge A_{2i}, A_{2i+1}$ for any $i$. Therefore, we will output the values of $T$ in reverse order.
      \begin{algorithm}
